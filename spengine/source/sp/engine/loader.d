...
public:
    ...
    Mesh getMesh( string name )
    {
        ...
        throw new SPLoaderException( "no mesh '" ~ name ~
                                     "' in " ~ sourceName );
    }

    ///
    Mesh getMesh( size_t no )
    {
        if( no < scene.mNumMeshes )
            return convMesh( scene.mMeshes[no] );
        throw new SPLoaderException( "no mesh #" ~ to!string(no) ~
                                     " in " ~ sourceName );
    }

protected:

    Mesh convMesh( in aiMesh* m )
    ...
    TexCoord[] getTexCoords( in aiMesh* m )
    {
        TexCoord[] ret;
